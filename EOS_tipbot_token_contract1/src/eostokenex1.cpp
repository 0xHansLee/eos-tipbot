...
#include <eostokenex1.hpp>



ACTION eostokenex1::create( name   issuer,
...
ACTION eostokenex1::issue( name to, asset quantity, string memo )
{
...
ACTION eostokenex1::retire( asset quantity, string memo )
{
    auto sym = quantity.symbol;
    eosio::check( sym.is_valid(), "invalid symbol name" );
    eosio::check( memo.size() <= 256, "memo has more than 256 bytes" );
...
ACTION eostokenex1::transfer( name    from,
                      name    to,
                      asset   quantity,
                      string  memo )
{
...
void eostokenex1::sub_balance( name owner, asset value ) {
...
void eostokenex1::add_balance( name owner, asset value, name ram_payer )
{
...
ACTION eostokenex1::open( name owner, const symbol& symbol, name ram_payer )
{
...
ACTION eostokenex1::close( name owner, const symbol& symbol )
{
   require_auth( owner );
   accounts acnts( _self, owner.value );
...
EOSIO_DISPATCH( eostokenex1, (create)(issue)(transfer)(open)(close)(retire) )
